...
// strings_to_list(4, powers, &list_one, &list_two)
// struct node *strings_to_list(int len, char *strings[], struct node **list1, struct node **list2);
...
struct node *strings_to_list(int len, char *strings[]) {
  ...
  struct node *string_list_head = create_list(atoi(strings[0]));
  unsigned int string_index = 1;
  struct node *curr = string_list_head;
  while (string_index < len) {
    int string_value = atoi(strings[string_index]);
    struct node *new_node = create_node(string_value);
    curr->next = new_node;
    curr = curr->next;
    string_index += 1;
  }
