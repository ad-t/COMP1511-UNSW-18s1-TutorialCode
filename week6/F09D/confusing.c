...
#define EXPECTED_SCANF_INPUTS 8
...
int main(int argc, char* argv[]) {
  ...
  // print error
  if( scanfsReturnValueAggregatedOverAllScanfs != EXPECTED_SCANF_INPUTS ) {
    printf("Error in collecting inputs. Expected %d\n", EXPECTED_SCANF_INPUTS);
    return 1;
  }
...
// tests the point (px, py) whether it lies above, below or on the line
// described my slope m and y-intercept b
int test2( double px, double py, double m, double b ) {   
  if( py < m * px + b ) {
...
int tritest(double x0,double y0,double x1,double y1,double x2,double y2,double px, double py){
  ...
  // Point-gradient formula use for the triangle tests later on
  // b: y - y1 = m( x - x1 ), x = 0
